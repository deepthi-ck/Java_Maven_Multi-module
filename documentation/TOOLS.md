# Integrated quality tools (Java 8)

These tools are wired into this Maven multi-module project via the parent `pom.xml`, `config/`, and `scripts/`.
...
Java toolchain: `maven-compiler-plugin` (`source/target=1.8`), Surefire, Spring Boot Maven plugin (`repackage` / `run`).